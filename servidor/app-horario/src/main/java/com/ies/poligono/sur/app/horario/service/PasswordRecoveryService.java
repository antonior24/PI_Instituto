...
public class PasswordRecoveryService {
    ...
    public PasswordRecoveryResult recoverPassword(String email) {
        ...
        logger.info("Password recovery requested. mode={}, resendConfigured={}, email={}",
                mode, configuredSender, email);

        if (!testingMode && !configuredSender) {
            logger.error("Password recovery cannot continue because production mode requires Resend configuration");
        ...
        if (!testingMode) {
            logger.info("Attempting to send password recovery email via Resend to {}", usuario.getEmail());
